...
ProjectManager.prototype.ensureSdkIsInstalled = function ensureSdkIsInstalled() {
	...
	return this.executeCommand('titanium', [ 'sdk', 'list' ])
		.then(result => {
			if (result.stdout.indexOf(this.options.sdkVersion) !== -1) {
				this.logger.debug(`Requested SDK version ${this.options.sdkVersion} is installed.`);
				return Promise.resolve();
			} else {
